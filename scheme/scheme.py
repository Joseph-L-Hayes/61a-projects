...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        #use first and rest from above
        operator = scheme_eval(first, env) #first expr should be operator
        check_procedure(operator) #interrupts the program by raising error?
        operands = rest.map(lambda pair: scheme_eval(pair, env))
        #had trouble with map and rest
        return scheme_apply(operator, operands, env)
        #rest is a pair. map applies scheme_eval to each pair in rest pairs object
        #if check_procedure(expr):
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        return None
    elif expressions.second == nil: #if the second pair is empty return first Pair
        return scheme_eval(expressions.first, env)
    else:
        scheme_eval(expressions.first, env) #else eval the first pair and eval_all the rest
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            ...
            return self.parent.lookup(symbol) #need to return the value
    ...
    def rebind(self, symbol, value):
        ...
        #recursive: if there is a parent, call rebind on that parent
        #'looking up value' in directions is misleading; thought I had to
        #use the lookup function in the last step
        # END PROBLEM 3
        else:
            raise SchemeError('unknown identifier: {0}'.format(symbol))
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Invalid # of args in {0}'.format(vals))

        child = Frame(self)

        for item in range(0, len(formals)):
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        "*** YOUR CODE HERE ***"
        if self.use_env:
            python_args.append(env) #add env to list
            #using python.args += env got Frame object not iterable error
        try:
            return self.fn(*python_args)
        except TypeError:
            raise SchemeError(('Incorrect number of arguments for {0}: {1} given').format(self.fn, len(python_args)))
        # END PROBLEM 4
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env): #env is frame where procedure is called
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a lexically-scoped call evaluated in environment ENV."""
        # BEGIN PROBLEM 12
        "*** YOUR CODE HERE ***"
        defined_frame = self.env

        return defined_frame.make_child_frame(self.formals, args)
        # END PROBLEM 12
...
def do_define_form(expressions, env):
    ...
    check_form(expressions, 2) #checks that expression is a proper list

    target = expressions.first #first part of the Pair object

    if scheme_symbolp(target): #checks if target is a valid symbol
        check_form(expressions, 2, 2) #checks is a list min 2 max 2
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        value = scheme_eval(expressions.second.first, env)
        env.define(target, value)
        return target
        #Pair.second.first gives second then first values of the pair how?

    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        function_name = target.first #defined function name
        function_params = target.second
        function_body = expressions.second
        function = Pair(function_params, function_body)

        env.define(function_name, do_lambda_form(function, env))
...
#if the first thing in expressions is true, return the second thing
#if it's false, return the third thing; if it exists
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        target = scheme_eval(expressions.first, env)
        if scheme_falsep(target):
            return False
        elif scheme_truep(expressions.first) and expressions.second == nil:
            return target
        else:
            return do_and_form(expressions.second, env)

    #(and 1 #f) 1 is expressions.first and #f is expressions.second
    # END PROBLEM 13
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            "*** YOUR CODE HERE ***"
            if len(clause) == 1:
                return test #return predicate value if no corr sub-expr
            else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    def binding_helper(bindings, env, symbols, values):

        if bindings is not nil:
            check_form(bindings.first, 2, 2) #make sure bindings is pair with 2 things
            symbols = Pair(bindings.first.first, symbols) #add next symbol in bindings
            values = Pair(scheme_eval(bindings.first.second.first, env), values)

            return binding_helper(bindings.second, env, symbols, values)
            #move to next Pair in list and carry the symbols and values
        check_formals(symbols)
        return env.make_child_frame(symbols, values)

    return binding_helper(bindings, env, symbols=nil, values=nil)
    #helper function to allow recursive solution
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env): #env is frame where procedure is called
        ...
        eval_frame = env
        return eval_frame.make_child_frame(self.formals, args)
    # END PROBLEM 16
