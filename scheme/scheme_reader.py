...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            #we've reached the end of the list or pair.
            #Remove this token from the buffer and return the nil object.
            src.remove_front() #missed this part in directions
            return nil
            # END PROBLEM 1
        elif src.current() == '.':
            # BEGIN PROBLEM 2
            "*** YOUR CODE HERE ***"
            src.remove_front() #remove dot
            next_expression = scheme_read(src)
            next_token = src.current()

            if next_token == ')':
                src.remove_front() #remove closing parens
                return next_expression
            else:
                raise SyntaxError('expected one expression after .')
            #next_expression evals to the thing after dot
            #scheme_read calls remove on the first thing which means the next
            #time I call src.current(), I get the thing that follows 'expression'
            #next_token is the thing after the value in next_expression
