...
def key_of_min_value(d):
    ...
    """
    # BEGIN Question 0
    return min(d, key=lambda x: d[x]) #dictionary[x] = value at key x
    # END Question 0

def zip(*sequences): #combines first el in first list with first in next lists
...
def enumerate(s, start=0):
    ...
    >>> enumerate([6, 1, 'a'])
    [[0, 6], [1, 1], [2, 'a']]
    >>> enumerate('five', 5) #gave a string in place of a list
    [[5, 'f'], [6, 'i'], [7, 'v'], [8, 'e']]
    """
    # BEGIN Question 0
    "*** YOUR CODE HERE ***"
    return zip(range(start, start + len(s)), s)
    #range must have a stop value
    #range returns a list from start to end
    # END Question 0
...
def mean(s):
    ...
    # BEGIN Question 1
    "*** YOUR CODE HERE ***"
    assert len(s) > 0, 'Sequence is empty'
