    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.can_contain(insect): #ant here can contain the insect placed
                    self.ant.contained_ant = insect
                elif insect.can_contain(self.ant): #ant being placed can contain ant here
                    insect.contained_ant = self.ant #puts the ant here in the container
                    self.ant = insect #the container is the ant in the place
                else:
                    assert self.ant is None, 'Two ants in {0}'.format(self)
                # END Problem 9 #success! first try!
        else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        if self.place.ant is None or not self.place.ant.blocks_path: #True or not False for case of ninja
            return False
        return self.place.ant is not None #blocked returns True if there's an ant
        # END Problem 7
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.is_container and not other.is_container:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        i, moves, start_place = 0, 0, self.place

        while i < self.min_range: #modifies start_pos based on min_range
            start_place = start_place.entrance #should move to next object
            i += 1

        while start_place is not hive and moves <= self.max_range: #and not max_range
            if start_place.bees: #bees in current position has bees in it
                return random_or_none(start_place.bees) #was using self.place.bees
        ...
        return None
        #bees is a list of Bees
        #random_or_none returns a random element of a sequence
        #so this will return a bee from bees in the same sqaure as the ant
        # END Problem 3 and 4
...
class LongThrower(ThrowerAnt):
    ...
    name = 'Long'
    food_cost = 2
    min_range = 5 #self.place + 5 entrances
    max_range = float('inf')
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUI
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        ant_place = self.place #need to store the place before the ant dies
        Insect.reduce_armor(self, amount) #reduce ant's armor

        if self.armor <= 0: #damage the bees if armor is <=0
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        bee.reduce_armor(bee.armor) #damages by exactly the bee's armor value!
        # END Problem 6
    ...
    def action(self, colony):
        ...
        if not self.digesting and self.place.bees:
            ...
            self.digesting -= 1 #arggggg this was =- ..
        # END Problem 6
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn and blocks the path"""
    ...
    def __init__(self, armor=4):
        Ant.__init__(self, armor) #success!
# END Problem 8
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        NinjaAnt.action(self, colony) #NinjaAnt does the same action
        # END Problem 10
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_watersafe:
            Insect.reduce_armor(insect, insect.armor) #self, amount of damage
        # END Problem 11

# BEGIN Problem 12
# The ScubaThrower class
class ScubaThrower(ThrowerAnt):
    """Scuba is a thrower ant that can be placed on water"""
...
# END Problem 13
    ...
    food_cost = 7
    Queen_exists = True
     #a list of ant instances that have been buffed
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 13
    implemented = True   # Change to True to view in the GUI
    # END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.Queen_exists: #if there is not already a Queen, this is the Queen
            self.true_Queen = True
            self.buffed_ants = []
            QueenAnt.Queen_exists = False #the true Queen is already here

        else:
            self.true_Queen = False #this Queen will be an imposter
        # END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if not self.true_Queen: #imposter queen action
            self.reduce_armor(self.armor) #reduce armor to 0 and remove

        else: #true Queen action
            ThrowerAnt.action(self, colony)
            start_place = self.place.exit #start at the queens place exit

            while start_place is not None:

                if start_place.ant and start_place.ant not in self.buffed_ants:
                    #buffs the container ant
                    start_place.ant.damage *= 2
                    self.buffed_ants.append(start_place.ant)

                if start_place.ant and start_place.ant.is_container and start_place.ant.contained_ant:
                    if start_place.ant.contained_ant not in self.buffed_ants:
                        #if ant is container, buffs contained ant
                        start_place.ant.contained_ant.damage *= 2
                        self.buffed_ants.append(start_place.ant.contained_ant)
